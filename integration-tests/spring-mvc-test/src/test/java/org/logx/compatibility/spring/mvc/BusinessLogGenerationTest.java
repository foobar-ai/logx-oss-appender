...
/**
 * Business log generation scenarios for the Spring MVC stack using log4j2.
 *
 * <p>The scenarios align with the Spring Boot compatibility suite and cover trigger thresholds,
 * OSS connectivity and QPS baselines while keeping the workload suitable for CI executions.</p>
 */
public class BusinessLogGenerationTest {

    private static final Logger LOGGER = LogManager.getLogger(BusinessLogGenerationTest.class);
    private final BusinessLogScenario scenario = new BusinessLogScenario(new Log4j2Facade(LOGGER));
    ...
    private static final class BusinessLogScenario {
        ...
        private void runOssConnectivityDiagnostics() throws InterruptedException {
            ...
            for (int i = 1; i <= 20; i++) {
                logger.info(String.format(Locale.US,
                        "OSS diagnostic log #%d - controller=OrderController, timestamp=%d",
            ...
            for (int i = 1; i <= 120; i++) {
                logger.info(String.format(Locale.US,
                        "Batch trigger log #%d - controller=AuditController, action=push", i));
                if (i % 15 == 0) {
                    TimeUnit.MILLISECONDS.sleep(35);
                }
            }

            logger.info("OSS diagnostics completed. Verify artifacts in the target bucket if available.");
        }
        ...
        private void runQpsPerformanceProbe(int messageCount, int expectedQps) {
            ...
            for (int i = 0; i < messageCount; i++) {
                logger.info(String.format(Locale.US,
                        "QPS probe #%d - controller=MetricsController, traceId=%s, latency=%d",
                        i + 1,
            ...
            if (actualQps < expectedQps) {
                logger.warn(String.format(Locale.US,
                        "Observed QPS %.0f is below target %d. Review dispatcher thread pools.",
        ...
        private void simulateMessageCountTrigger(int messageTarget) throws InterruptedException {
            ...
            for (int i = 0; i < messageTarget; i++) {
                logger.info(String.format(Locale.US,
                        "Count trigger #%d - controller=OrderController, sequence=%05d, ts=%d",
                        i + 1,
        ...
        private void simulateMessageAgeTrigger() throws InterruptedException {
            ...
            for (int i = 0; i < 80; i++) {
                logger.info(String.format(Locale.US,
                        "Age trigger #%d - controller=AuditController, status=%s, ts=%d",
                        i + 1,
                        randomStatus(),
                        System.currentTimeMillis()));
                if ((i + 1) % 20 == 0) {
        ...
        private String buildLargeMessage() {
            String[] words = {
                    "controller", "service", "repository", "cache", "queue", "filter", "interceptor",
                    "validator", "handler", "pipeline", "websocket", "metrics", "snapshot", "config",
                    "tenant", "region", "locale", "payload", "checksum", "routing"
            };
